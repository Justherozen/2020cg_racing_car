...
public:
	...
	void ForwardEffect();//远离特效
	void BackEffect();//靠近特效
	void ZoomRecover();//视角恢复
...
//private:
    ...
    float yaw = -90.0f;//水平偏斜
    float pitch = 0.0f;//竖直偏斜
	glm::vec3 cameraFront = glm::vec3(0.0f, 0.0f, -3.0f);
	glm::vec3 cameraUp = glm::vec3(0.0f, 1.0f, 0.0f);
	float fov = 80.0f;//视角
	float sensitivity = 0.05;//敏感度
	bool fixed = false;//固定
	float radius = 1.0f;
	float FixedYaw = 0.0f;//固定时的旋转角
